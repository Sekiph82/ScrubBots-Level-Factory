...
def test_canonical_preview_depth_hardening_is_real_and_authority_bound() -> None:
    parent = _parent()
    request = MutationRequest.for_candidate(parent, operator_id="CANONICAL_PREVIEW_DEPTH_HARDEN_V1", operator_version="1", seed=11, intent=MutationIntent.HARDEN, authority=CANONICAL_M23_PREVIEW_AUTHORITY)
...
def test_hardening_is_inapplicable_at_bound_and_does_not_guess() -> None:
    parent = _parent(4)
    request = MutationRequest.for_candidate(parent, operator_id="CANONICAL_PREVIEW_DEPTH_HARDEN_V1", operator_version="1", seed=12, intent=MutationIntent.HARDEN, authority=CANONICAL_M23_PREVIEW_AUTHORITY)
    result = MutationEngine().apply(request, parent)
...
def test_hardening_cannot_use_dimensions_or_color_count_as_a_shortcut() -> None:
    ...
    request = MutationRequest.for_candidate(parent, operator_id="CANONICAL_PREVIEW_DEPTH_HARDEN_V1", operator_version="1", seed=13, intent=MutationIntent.HARDEN, authority=CANONICAL_M23_PREVIEW_AUTHORITY)
    result = MutationEngine().apply(request, parent)
    assert result.disposition is MutationDisposition.APPLIED
    assert result.child is not None
