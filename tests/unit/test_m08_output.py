...
def test_artwork_json_contract_and_rectangular_row_major_index() -> None:
    cells = ["C01", "C02", "C03", "C01", "C02", "C03"]
    # Use a legal EASY artifact for the public contract; the small helper below
    # is reserved for the PNG codec's profile tests.
    artifact = ArtworkArtifact.from_cells("rectangular", "EASY", 20, 20, _cells())
    raw = artifact.as_dict()
    assert raw["index_rule"] == "index = y * width + x"
    assert raw["cells"][0] == "C01"
    assert raw["cells"][19] == "C01"
    assert raw["cells"][20] == "C01"
