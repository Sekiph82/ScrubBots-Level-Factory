...
def test_m03_m04_m05_m06_results_can_be_analyzed_without_private_topology() -> None:
    ...
    for result in results:
        ...
        report = evaluate_grid(result.width, result.height, result.logical_grid)
...
def test_m05_wfc_and_m06_auto_results_are_compatible_with_grid_only_quality() -> None:
    ...
    assert evaluate_grid(wfc_result.width, wfc_result.height, wfc_result.logical_grid, policy=QualityPolicy(max_isolated_ratio=1.0, max_tiny_cell_ratio=1.0, max_color_dominance_ratio=1.0)).analysis is not None
    ...
    assert evaluate_grid(auto_result.width, auto_result.height, auto_result.logical_grid).analysis is not None
