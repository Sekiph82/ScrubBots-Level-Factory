# SB-LF03-005-C001 — Canonical Visited-State Memoization / Hashing — Strict Audit
...
- Level Factory implementation commit: `c09a8c2e92810346f823b44ad895139790c2ef5b`
- terminal builder-log commit: `17d005940ab6fdedb09e36920a7e6802ab2a398f`
...
## MAJOR-001 — Opaque key evidence is not bound back to the state passed to the key provider

`StateKeyResult` carries `state_digest`, but neither `DeterministicVisitedMemo.observe()` nor the evidence collector verifies that this digest equals the digest of the state for which `key(state)` was requested.

A provider can therefore return a canonical key result for state B while being queried for state A. If provider/authority strings otherwise match, the memo accepts the key and can collapse the wrong search state.

That breaks fail-closed semantic memoization.
...
Introduce a state/result binding validator, or change memo observation to receive the queried state together with the result. Require exact state digest + authority + provider identity match before FIRST_VISIT/MEMO_HIT. Update all consumers. Add a negative test where a syntactically valid key result is deliberately bound to another compact state.
