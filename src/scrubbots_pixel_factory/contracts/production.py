...
PRODUCTION_DIMENSION_MIN = PRODUCTION_DIMENSION_ENVELOPE.minimum
PRODUCTION_DIMENSION_MAX = PRODUCTION_DIMENSION_ENVELOPE.maximum
...
def validate_production_width(width: int) -> int:
    if not PRODUCTION_DIMENSION_ENVELOPE.contains(width):
...
def validate_production_height(height: int) -> int:
    if not PRODUCTION_DIMENSION_ENVELOPE.contains(height):
        raise ProductionContractError(
