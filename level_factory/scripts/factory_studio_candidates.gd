...
func _render() -> void:
	...
	if _details_label != null:
		if selected.is_empty():
			...
			_details_label.text = "Selected candidate evidence: id=%s | artwork=%s | grid=%s | dimensions=%sx%s | provenance=%s | structural/QA=%s | similarity=%s | owner-review=%s | solver=%s | difficulty=%s | immutable references=%s | invalid review=%s" % [selected.get("candidate_id", ""), selected.get("artwork_sha256", ""), selected.get("grid_hash", ""), selected.get("width", ""), selected.get("height", ""), selected.get("provenance", {}), selected.get("structural", {}).get("decision", selected.get("structural", {}).get("disposition", "NOT AVAILABLE")), selected.get("similarity_advisory", {}).get("disposition", "NOT AVAILABLE"), selected.get("owner_review", {}).get("disposition", "NEEDS_REVIEW"), selected.get("solver", {}).get("disposition", "NOT AVAILABLE"), selected.get("difficulty", {}).get("disposition", "NOT AVAILABLE"), selected.get("evidence_references", []), selected.get("owner_review_invalid", [])]
