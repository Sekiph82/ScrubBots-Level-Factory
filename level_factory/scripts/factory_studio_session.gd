...
func save_session() -> void:
	_projection = _gateway.call("run_studio_extension", "session-save", {"session_id": _session.text.strip_edges(), "state": {"surface": "Session", "active_batch_id": "batch-runtime", "nested": {"api_key": "must-not-persist", "note": "saved"}}}) if _gateway != null else {"state": "UNAVAILABLE"}; _render()
