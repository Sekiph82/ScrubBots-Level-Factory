...
func _ready() -> void:
	if get_child_count() == 0:
		...
		var row := HBoxContainer.new(); _candidate = LineEdit.new(); _candidate.placeholder_text = "candidate ID"; _candidate.size_flags_horizontal = Control.SIZE_EXPAND_FILL; row.add_child(_candidate); var button := Button.new(); button.text = "Check capability"; button.pressed.connect(check_capability); row.add_child(button); var reproduce := Button.new(); reproduce.text = "Exact Reproduce"; reproduce.pressed.connect(exact_reproduce); row.add_child(reproduce); add_child(row)
...
func exact_reproduce() -> void: _projection = _gateway.call("run_studio_extension", "reproduce-exact", {"candidate_id": _candidate.text.strip_edges()}) if _gateway != null else {"state": "UNAVAILABLE"}; _render()
func snapshot() -> Dictionary: return _projection.duplicate(true)
func show_reproduce() -> void: _render()
func _render() -> void:
	if _result != null: _result.text = "Capability: %s\n%s\nOutput: %s" % [_projection.get("disposition", "NOT AVAILABLE"), _projection.get("reason", _projection.get("error", "Exact replay is disabled until a real recorded path exists.")), _projection.get("output_path", "not executed")]
