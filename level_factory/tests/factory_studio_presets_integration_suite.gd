...
func _run_suite() -> void:
	...
	_require(_request_equal(execution_a.get("expanded_request", {}), settings_a), "execution request differs from expanded preset request: %s" % execution_a)
	...
	_require(_request_equal(reapplied.get("execution", {}).get("expanded_request", {}), updated_settings), "updated preset did not affect the new execution: %s" % reapplied)
