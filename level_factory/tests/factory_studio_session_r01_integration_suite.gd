...
func _run_suite() -> void:
	...
	var batch: Dictionary = gateway.call("run_studio_extension", "batch-import", {"paths": [source_path]}); var session_source_id := str(batch.get("items", [])[0].get("source_id", "")); var pipeline: Dictionary = gateway.call("run_studio_extension", "pipeline", {"source_id": session_source_id}); var run_id := str(pipeline.get("run_id", ""))
	...
	var restored: Dictionary = gateway.call("run_studio_extension", "session-restore", {"session_id": "lfx015-runtime"}); _require(restored.get("recovery") == "RESUMED" and restored.get("validated_references") == true, "real durable session did not resume with validated references: %s" % restored)
