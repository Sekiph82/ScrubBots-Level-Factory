...
func _run_suite() -> void:
	...
	var restored: Dictionary = surface_after_restart.call("snapshot").get("projection", {}); _require(restored.get("recovery") == "NOT_RESUMABLE" and restored.get("validated_references") == true, "fresh Studio did not report truthful recovery: %s" % restored)
	_require(restored.get("recovery_execution", {}).get("disposition") == "NOT_RESUMABLE", "recovery coordinator overstated unavailable continuation")
	...
	_require(restored.get("recovery_execution", {}).get("continued_stage_evidence") == null and restored.get("recovery_execution", {}).get("next_stage") == "SOLVE" and restored.get("recovery_execution", {}).get("next_stage_disposition") == "NOT_AVAILABLE", "recovery fabricated a continuation stage")
	...
	var restored_again: Dictionary = gateway_after_restart.call("run_studio_extension", "session-restore", {"session_id": "lfx015-runtime"}); _require(restored_again.get("recovery") == "NOT_RESUMABLE" and restored_again.get("recovery_execution", {}).get("disposition") == "NOT_RESUMABLE", "repeated recovery changed the durable non-resumable disposition")
...
func _cleanup(instance: Node) -> void:
	_remove_tree(ProjectSettings.globalize_path("res://output/studio-extensions")); _remove_tree(ProjectSettings.globalize_path("res://output/owner-uploads")); _remove_tree(ProjectSettings.globalize_path("res://output/.lfx015-candidate")); instance.queue_free(); _finish()
