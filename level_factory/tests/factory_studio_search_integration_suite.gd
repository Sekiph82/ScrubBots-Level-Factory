...
func _run_suite() -> void:
	...
	var accepted: Dictionary = gateway.call("run_studio_extension", "discover", {"collection": "Owner Accepted"})
	_require(accepted.get("records", []).any(func(record): return record.get("record_id") == left_id), "Owner Accepted collection omitted accepted candidate before later review changes")
	var color_count: Dictionary = gateway.call("run_studio_extension", "discover", {"filters": {"record_type": "CANDIDATE", "used_color_count": int(combined.get("records", [])[0].get("used_color_count", 0))}})
	_require(color_count.get("state") == "READY" and color_count.get("records", []).size() >= 1, "used_color_count filter did not match its advertised candidate field")
	...
	var unused: Dictionary = gateway.call("run_studio_extension", "discover", {"collection": "Unused in Campaign"})
	_require(unused.get("state") == "NOT AVAILABLE" and unused.get("records", []).is_empty(), "Unused in Campaign was not reported as unavailable")
	var repeat_a: Dictionary = gateway.call("run_studio_extension", "discover", {"query": "", "filters": {"record_type": "CANDIDATE"}})
	var repeat_b: Dictionary = gateway.call("run_studio_extension", "discover", {"query": "", "filters": {"record_type": "CANDIDATE"}})
	_require(repeat_a.get("records", []) == repeat_b.get("records", []), "repeated identical discovery query changed deterministic order/content")
